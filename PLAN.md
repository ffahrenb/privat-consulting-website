...
**`docs/CV_CROSSCHECK.md`** — the audit, reported not acted on. Highlights:
- **"Psychologe FSP"** — the primary professional title on *both* sites, in Impressum, Kontakt and About — **appears nowhere in the CV.** Not in certifications, education or honors. The single most load-bearing credential and the CV is silent.
- **"SAC-Tourenleiter"** (psych, presented as a qualification) does not appear in the CV either; the CV certifies **DSV ski instructor** and lists climbing instruction as a **volunteer role**.
- **The Learning Organization citation**: sites say `2019, 27(6), 499–512`; `.bib` says `2022, 29(6), 499–517`. Same DOI — one is simply wrong, and the error is repeated on both sites.
- **MSc Cognitive Science** is credited to *Ljubljana* on the sites; the CV says **Med Uni Vienna** (Ljubljana was a separate visiting semester).
- Author order swapped on Kaiser/Kragulj/Fahrenbach 2017; master-level teaching claimed but CV says bachelor-only; Prof. Nater / Prof. Kaiser / "Theory Wave" unsupported; the flagship VERITAS & BI-Clarity case-study numbers have **no CV corroboration at all** (consultancy entries stop in 2021, no Swiss clients).
- **Missed opportunities:** 5 of 17 refereed papers used, 0 of 10 talks, 0 of 12 named consultancy engagements, 1 of 7 awards. Biggest single miss — the consulting homepage's central thesis is *"soziotechnische Systeme"* and he has an **uncited peer-reviewed journal article on socio-technical perspectives** (Fahrenbach & Luomi-Messerer 2022).

**`docs/DESIGN_BRIEF.md`** — the instruction doc for a design pass. Covers the current state's actual gaps (no `og:`/`twitter:`/`canonical`/JSON-LD on either site; no skip link; hamburger has no `aria-expanded`; `--color-border: #d4cfc9` is a warm beige stranded in consulting's cool palette; no shadow/elevation tokens — every shadow is a hardcoded rgba; type scale is flat), the two-brand token system, component-level direction, and — per your request — **folds the CV in as a design problem**: how to surface publications, talks, awards and a proper credentials block without turning the site into a CV dump.
